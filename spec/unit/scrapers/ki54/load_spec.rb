...
describe Scrapers::Ki54 do
  ...
  describe '#load!' do
    ...
    context 'with index.html load', vcr: { cassette_name: "#{vcr_base}/index", match_requests_on: [:path] } do
      ...
      context 'for Tachikawa_Ki-54' do
        let(:expected_uuid) { '727a97f8e20ed1600935feca47fbcad2' }
        it 'loads details correctly' do
          expect { subject }.to change { component.catalog.planes[expected_uuid]&.keys&.sort }.from(nil).to(expected_keys.sort)
